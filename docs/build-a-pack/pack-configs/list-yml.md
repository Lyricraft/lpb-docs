...
资源列表（list.yml）是一个 YAML 文件，位于 `lpb/list.yml`，定义整合包中包含的模组、资源包、数据包、附加文件等内容，以及应用它们的条件。同时也定义可能使用的特定文件夹，如数据包文件夹。
...
此外，可以专门定义特殊的文件夹 id，它们以 `builder:` 加上平台资源内容类型开头，比如 `builder:shader`。定义它们会影响 LPB 在应用平台资源时的行为，不把对应资源放在默认的文件夹下而放在此处定义的文件夹内。可前往 [平台资源类型](./resources/platform-resource.md) 了解详情。
