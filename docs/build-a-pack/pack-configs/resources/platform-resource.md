...
link 是可以不填写的，但如果不填写 link，则 versions 必须填写。而且 choice 必须设为 static。如果 choice 也被省略，则会自动设为 static。这在已手动指定所有版本文件，无需自动选择时可以用到。如果条件版本没有一项满足，则停止构建并抛出错误。
...
- 版本 id（id），如 `pXyXLdlT`。
- 版本号（version_number），如 `mc1.21.11-0.8.7-neoforge`。
- 版本详情页 URL，如 ` https://modrinth.com/mod/sodium/version/pXyXLdlT ` 和 ` https://modrinth.com/mod/sodium/version/mc1.21.11-0.8.7-neoforge `。

在使用 CurseForge 资源时，版本号实际可以填写这些值：

- 文件 id（id），如 `7805160`。使用时请用引号包裹，以保证被解析为字符串。
- 文件详情页 URL，如 ` https://www.curseforge.com/minecraft/mc-mods/sodium/files/7805160 `。

二者都可以使用文件下载直链。如 Modrinth 的 ` https://cdn.modrinth.com/data/AANobbMI/versions/UddlN6L4/sodium-fabric-0.8.7%2Bmc1.21.11.jar ` 和 CurseForge 的 ` https://mediafilez.forgecdn.net/files/7805/160/sodium-neoforge-0.8.7%2Bmc1.21.11.jar `。事实上，任何一个下载文件的直链都可以填写在这里，即使它们不是来自 Modrinth 或 CurseForge。LPB 不会检查链接来源，而是在满足条件时直接将其作为下载链接。但是我们并不推荐这么做，这可能导致下载的文件不安全，或者整合包无法通过审核而无法上传至上述两个平台。
