...
- `builder:shaderpacks`: 默认指向 `shaderpacks` 目录，或者在资源列表 [list.yml](../list-yml.md) 中重新定义的值。应用平台资源类型时，如果没有专门指定位置，会自动将光影包放在此目录下。
- `builder:datapacks`: 无默认指向，如需要使用应当在资源列表 list.yml 中定义。应用平台资源类型时，如果没有专门指定位置，会自动将数据包放在此目录下。如果没有定义，则停止构建并抛出错误。
- `list:定义的文件夹 id`: 指向资源列表 list.yml 中定义的位置。

## 资源应用名称（rename）

资源应用名称指定资源文件在应用时会被命名为什么。这个字段可以省略，将自动使用默认名字，比如 modrinth 上下载的模组文件原名，以及本地文件原名。